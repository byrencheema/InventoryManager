...
export default function Home() {
  ...
  const updateInventory = async () => {
    ...
    const inventoryList = []
    docs.forEach(doc => {
      inventoryList.push({ name: doc.id,
        ...doc.data() });
    });
  ...
  <Box width="100vw" height="100vh" display="flex" justifyContent="center" alignItems="center" gap={2}>
    <Modal open={open} onClose={handleClose}>
      <Box position="absolute" top='50%' left='50%' bgcolor="white" p={2} borderRadius={2} width={400} border="2px solid #00000" p={4} display="flex" flexDirection={"column"} gap={3} sx={{transform: 'translate(-50%, -50%)'}}>
